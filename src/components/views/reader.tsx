...
export function ReaderView() {
  ...
  async function makeFlashcard(highlightId?: string) {
    ...
    try {
      ...
      toast({ title: 'Flashcard created — see Flashcards' })
    } catch {
    ...
    try {
      if (mindmapChoice === '__new') {
        ...
        if (res.existing) {
          ...
          toast({ title: 'Mindmap already exists' })
        } else {
          ...
          <div className="space-y-3 px-4 py-4">
            {(doc?.highlights.length ?? 0) === 0 ? (
              <p className="text-sm text-muted-foreground">
                {isPdf && mode === 'original' ? (
                  <>
                    Switch to <button onClick={() => setMode('text')} className="font-semibold underline underline-offset-2">Text mode</button>, select any passage and pick a color — highlights show up here.
                  </>
                ) : (
      ...
      <header className="flex flex-wrap items-center gap-2 border-b pb-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="truncate text-xs text-muted-foreground">
            ...
            {isPdf ? `${doc?.pageCount ?? '?'} pages` : doc?.type}
          </p>
        </div>
