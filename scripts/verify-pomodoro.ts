...
// ── 4. work boundary → short break, logged once ──────────────────────
console.log('4. work boundary')
const t0 = st().endAt!
st().tick(t0 + 1000) // work block completed while away
check('advanced to short break', st().phase === 'short')
check('completed = 1', st().completed === 1)
check('auto-start break (default)', st().running && st().endAt !== null)
...
check('work block logged', logCalls.length === 1 && logCalls[0].minutes === 25, JSON.stringify(logCalls))
...
// ── 5. break boundary → back to work (auto); long absence is bounded ──
console.log('5. break boundary & long absence')
const bEnd = st().endAt!
st().tick(bEnd + 1000)
check('back to work after break', st().phase === 'work' && st().completed === 1)
...
// logs ONCE (the phase that was running when the user left); the break after
// it would also already be over → parked at its start, never chained further
...
check('absence parks at next phase start', !st().running && st().endAt === null && st().secondsLeft === 5 * 60 && st().phase === 'short', `running=${st().running} phase=${st().phase} left=${st().secondsLeft}`)
check('focusSeconds = two full blocks', st().focusSeconds === 50 * 60, `got ${st().focusSeconds}`)

// ── 6. skip = advance without logging ────────────────────────────────
console.log('6. skip')
st().skip() // skip the parked short break → work
check('skip break returns to work', st().phase === 'work' && logCalls.length === loggedAfterFirst + 1)
st().skip() // skip the untouched work block → short, no log
check('skip advances work without logging', st().phase === 'short' && logCalls.length === loggedAfterFirst + 1 && st().completed === 3)

// ── 7. stop with partial work block logs the remainder ───────────────
console.log('7. stop & partial log')
// get into a running work phase: finish the running short break, then run 10
// minutes of the auto-started work block and end — must log exactly 10 min
const shortEnd = st().endAt!
st().tick(shortEnd + 1000)
check('work auto-started after break', st().phase === 'work' && st().running)
...
check('stop resets everything', !st().running && st().completed === 0 && st().phaseElapsed === 0 && st().taskLink === null)

// ── 8. custom durations + auto-start toggles ─────────────────────────
console.log('8. custom durations')
st().updateSettings({ workMin: 42, shortMin: 7, longMin: 21, roundsBeforeLong: 2, autoStartBreaks: false })
check('idle settings update resets countdown', st().secondsLeft === 42 * 60)
st().start()
check('custom focus length', st().secondsLeft === 42 * 60)
const wEnd = st().endAt!
st().tick(wEnd + 1000)
check('custom short break queued', st().phase === 'short' && st().secondsLeft === 7 * 60)
check('auto-start OFF → parked', !st().running && st().endAt === null && st().pausedRemaining === 7 * 60)
check('custom work block logged', logCalls.some((c) => c.minutes === 42))
st().resume(Date.now())
const sEnd = st().endAt!
st().tick(sEnd + 1000) // short break over → back to work (completed still 1)
check('back to work after short', st().phase === 'work' && st().secondsLeft === 42 * 60)
const w2End = st().endAt!
st().tick(w2End + 1000) // second work block over → completed=2 → LONG (rounds=2)
check('roundsBeforeLong=2 → long break', st().phase === 'long' && st().completed === 2, `phase=${st().phase} completed=${st().completed}`)
check('custom long break length', st().secondsLeft === 21 * 60)
st().stop()

// ── 9. mid-session start(task) switches the link, keeps the timer ────
console.log('9. task link switch')
st().updateSettings({ autoStartBreaks: true })
st().start({ id: 'task-a', title: 'Task A' })
const runEnd = st().endAt!
st().tick(runEnd - 10 * 60 * 1000) // 15 min into the block
st().start({ id: 'task-b', title: 'Task B' })
check('mid-session start keeps the countdown', st().running && st().endAt === runEnd && st().secondsLeft === 10 * 60)
